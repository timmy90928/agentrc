...
放 Codex CLI subagent 定義(**`*.toml`**,一檔一 agent);`install` 複製到 `~/.codex/agents/`。**目前為空 scaffold**(本機尚無自訂 agent)。
...
全域並發/深度設定另在 `~/.codex/config.toml` 的 `[agents]` 表(`max_threads` / `max_depth` / `job_max_runtime_seconds`)。叫用:CLI 內 `/agent`。
