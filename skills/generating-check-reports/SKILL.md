...
- **標籤覆寫 / 擴充**:`labels`(`status`/`severity`/`tabs`)覆寫既有固定字;`statuses` 定義**新狀態值**(自帶 label/color/role,role 決定算通過/未過/中性);`items[].tags` 給每項掛自由標籤(純標記不計分)。`category` 本就可自由命名。詳見 `references/results-schema.md`。
