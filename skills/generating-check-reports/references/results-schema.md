...
> **前後文(論文 / 文件修改場景)**:當文中同樣的字出現多次(如多個「18%」),只給 `from/to` 無法定位。補上 `before`/`after`,報告會渲染成 **原文 / 改為** 兩行,把改動的字嵌進上下文高亮——例如 `before:"A 組通過率為 "`、`from:"18%"`、`to:"20%"`、`after:",高於對照組"` → 原文「A 組通過率為 ~~18%~~,高於對照組」/ 改為「A 組通過率為 **20%**,高於對照組」。
