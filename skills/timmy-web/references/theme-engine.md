...
8. **金鑰驗證(兩段)**:
   - **填金鑰當下(失焦 `change`)**:`loadModels(true)` 回傳 Promise(成功 resolve 模型陣列、失敗 resolve `null`);成功 → 顯示 `chat.valid`(✓)、**即時 `state.key=key; save()` 存入金鑰**、並呼叫 `primeStarters()`;失敗 → `chat.keybad`(✗)。list-models 同時兼任「金鑰可用驗證」與「列出可用模型」,一次呼叫雙效。
   - **按「儲存」(validate-on-save)**:再用 `PROVIDERS[].send` 發一個最小生成請求(prompt `'Hi'`)確認「金鑰 + 模型 + 生成 + CORS」,**通過才寫 model**(綠 ✓、自動收合);失敗顯示原因不存(紅字)。i18n:`chat.validating` / `chat.valid` / `chat.invalid` / `chat.keybad` / `chat.nokey_save`。
...
11. **推薦問題(suggested questions)**:共用 `requestSuggestions()` —— 以 `pageContext()`(`document.title` + 前 6 個 h1/h2)+ 最近 6 turn 對話組 prompt,請 AI 生成「exactly 3」個追問;回覆按行 split、去前綴編號/符號後取前 3。渲染成 `.twc-chip` 可點鈕(點擊 = 填入並 `send()`);送出 / 清除對話時 `clearChips()`。三種觸發:
    - **起手問題(cold start・快取)**:`primeStarters()` 於**金鑰驗證通過後**呼叫;`showCachedStartersIfAny()` 先看 `state.starters`(`{page: pageId(), qs}`,存 `timmy-web-chat`)—— 本頁有快取且尚無對話 → 直接渲染(**0 API**);否則生成並寫入快取。載入時(`renderHistory()` 後)與清除對話後也呼叫 `showCachedStartersIfAny()`。`pageId()` = `location.pathname + '#' + document.title`(per-page 快取,避免同源各頁混用)。手動 💡 在「無對話」時走 `primeStarters(true)`(重生並更新快取)。
    - **自動追問**:`state.autosuggest`(預設 `true`,`.twc-autosug` checkbox 即時持久化、獨立於「儲存」鈕)為真時,**每次回覆成功後自動** `suggestQuestions(true)`(`silent=true` → 無金鑰/失敗靜默略過)。
    - **手動**:「💡 推薦問題」鈕(`.twc-sugbar`,`data-act="suggest"`,固定文字 `chat.suggest`)**僅在 `elChips` 為空時顯示**(`syncSugbar()` 於 `renderChips`/`clearChips` 切換 `.twc-sugbar` 的 `display`,避免「已有建議還放鈕」的重複);**有建議時改用問題 chips 後 append 的「🔄 換一批」`.twc-more`**(`chat.another`)。兩者都呼叫 `regenerate()`(對話中 `suggestQuestions()`、冷啟動 `primeStarters(true)`)。生成中鈕顯示「生成中…」(`sugBusy`),完成回 `chat.suggest`;切語言時 `.twc-more` 一併重譯。
    每次「生成」多一次 API(BYOK 額度);快取命中則 0 API。i18n:`chat.suggest` / `chat.suggesting` / `chat.autosuggest`。
12. **可開關 (opt-out)**:widget IIFE 開頭檢查 `window.TW_CONFIG.chat === false` 則直接 `return` 不注入(預設 undefined → 啟用)。供 `web-spec.md` Q10 選「不要」時,產出頁於 per-page `TW_CONFIG` 設 `chat:false` 停用;10 款示範頁皆預設啟用。
13. **per-page build-time 設定(`CFG = window.TW_CONFIG`)**:`chatStarters`(字串陣列・`cfgStarters()` 過濾非空字串、取前 4;設了則 `showCachedStartersIfAny()`/`primeStarters()` 直接顯示作者的、**不 AI 生成/快取**)、`chatAutoSuggest`(bool・`state.autosuggest` 初值)、`chatProvider`(`state.provider` 初值,`curProv()` 仍回退 gemini)、`chatModel`(`state.model` 初值)。皆於 state 初始化時套用,且**只當作預設**——訪客在面板改動後以 localStorage 持久值為準。未設則用引擎預設。其餘 `chatTitle`/`chatGreeting`/`chatSystem`/`chatScope` 仍規劃中。

**驗證**(`.claude/temp/verify_chatbot.py`,Playwright file://,34 項):FAB 出現、開面板、無金鑰自動開設定、FAB 開啟時可見且顯示 ✕、**填金鑰失焦即驗證 ✓ + 存入金鑰**、動態載入模型清單(過濾非對話模型)、**金鑰確認後備好起手問題 + 快取 localStorage + 重載由快取顯示(0 API)**、「自訂…」逃生口切換、儲存觸發驗證(mock 成功→存/失敗→不存)、選到的 model 寫入並用於送出、(mock fetch)送出渲染回覆、**推薦問題:回覆後自動生成 3 chips(預設開)+ 點 chip 送出 + 開關關閉後不自動 + 手動鈕仍可生成**、FAB 切換開關、深色適配、EN i18n、手機全螢幕、匯出檔不含 widget 且還原 live、**`TW_CONFIG.chat=false` 不注入 widget**、console 零錯誤。
