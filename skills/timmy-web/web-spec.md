...
#### 推薦問題
- **預設起手問題**(開啟即顯示的建議 chips,點了即送):
  - [ ] 不預設(只用「💡 推薦問題」即時生成)
  - [ ] 預設以下幾個(規劃中 → `TW_CONFIG.chatStarters`):
    1. ＿＿＿＿
    2. ＿＿＿＿
    3. ＿＿＿＿
- **自動推薦問題**(每次 AI 回覆後自動更新追問):
  - [ ] 開(預設;每輪多一次 API 呼叫、花訪客金鑰額度)
  - [ ] 關(只用手動「💡 推薦問題」鈕)
  > 現為訪客執行期開關(預設開、存其瀏覽器);build-time 預設值 規劃中 → `TW_CONFIG.chatAutoSuggest`
...
- **預設供應商**(訪客仍可在面板自行更換):
  ...
  - [ ] OpenAI
  - [ ] Claude (Anthropic)
  > build-time 預設 規劃中 → `TW_CONFIG.chatProvider` / `TW_CONFIG.chatModel`
- **金鑰策略**:
